...
CGameFramework::CGameFramework()
{
	...
	Define::SceneManager->LoadScene(0);
}
...
{
	//Define::SceneManager->GetCurrentScene()->OnProcessingMouseMessage(hWnd, nMessageID, wParam, lParam);
...
{
	//Define::SceneManager->GetCurrentScene()->OnProcessingKeyboardMessage(hWnd, nMessageID, wParam, lParam);
	Define::Input->OnProceesingKeyBoardMassage(hWnd, nMessageID, wParam, lParam);
	switch (nMessageID)
	{
	case WM_KEYDOWN:
		switch (wParam)
		{
		case VK_SPACE:
			//dynamic_cast<CTankPlayer*>(Define::Player)->FireBullet(NULL);
			break;
		default:
			break;
		}
...
{
	Define::SceneManager->GetCurrentScene()->ReleaseObjects();
}
