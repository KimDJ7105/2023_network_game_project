...
namespace Vector3
{
	inline float Distance(XMVECTOR& vec1, XMVECTOR vec2)
	{
		XMFLOAT3 xmf3Result;
		XMStoreFloat3(&xmf3Result, XMVector3Length(XMVectorSubtract(vec1, vec2)));
		return(xmf3Result.x);
	}

	inline float Distance(XMFLOAT3& xmf3Vector1, XMFLOAT3& xmf3Vector2)
	{
		return Distance(XMLoadFloat3(&xmf3Vector1), XMLoadFloat3(&xmf3Vector2));
	}

	inline float Distance(XMFLOAT3& xmf3Vector)
	{
		return Distance(xmf3Vector, XMFLOAT3(0, 0, 0));
	}


	inline XMFLOAT3 XM3fRandom(float min, float max)
	{
		std::uniform_real_distribution<float> dis(min, max);

		XMFLOAT3 randf3 = XMFLOAT3(dis(gen), dis(gen), dis(gen));
		return randf3;
	}

	inline XMFLOAT3 Multiply(XMFLOAT3& xmf3, float value)
	{
		XMFLOAT3 xmf3Result;
		xmf3Result.x = xmf3.x * value;
		xmf3Result.y = xmf3.y * value;
		xmf3Result.z = xmf3.z * value;

		return xmf3Result;
	}

	inline XMFLOAT3 Multiply(XMFLOAT3& a, XMFLOAT3& b)
	{
		XMFLOAT3 xmf3Result;
		xmf3Result.x = a.x * b.x;
		xmf3Result.y = a.y * b.y;
		xmf3Result.z = a.z * b.z;

		return xmf3Result;
	}

	inline XMFLOAT3 abs(XMFLOAT3& value)
	{
		XMFLOAT3 xmf3Result;
		xmf3Result.x = std::abs(value.x);
		xmf3Result.y = std::abs(value.y);
		xmf3Result.z = std::abs(value.z);
...
namespace Matrix4x4
{
	inline XMFLOAT4X4 Lerp(XMFLOAT4X4& v1, XMFLOAT4X4& v2, float t)
	{
		XMMATRIX matrix1 = XMLoadFloat4x4(&v1);
		XMMATRIX matrix2 = XMLoadFloat4x4(&v2);

		XMMATRIX value;
		value.r[0] = XMVectorLerp(matrix1.r[0], matrix2.r[0], t);
		value.r[1] = XMVectorLerp(matrix1.r[1], matrix2.r[1], t);
		value.r[2] = XMVectorLerp(matrix1.r[2], matrix2.r[2], t);
