...
void CMaterial::SetColor(XMVECTOR& color)
{
	if (m_pMaterialColors) XMStoreFloat4(&m_pMaterialColors->m_xmf4Diffuse, color);
}

XMVECTOR CMaterial::GetColor()
{
	XMFLOAT4 color = m_pMaterialColors->m_xmf4Diffuse;
...
void CGameObject::SetRoot()
{
	if (m_pSibling) {
		m_pSibling->root = this->root;
		m_pSibling->SetRoot();
	}
	if (m_pChild) {
		m_pChild->root = this->root;
		m_pChild->SetRoot();
	}
}
...
{
	CMesh* mesh = nullptr;
	if (m_pMesh) mesh = m_pMesh;

	if (m_pSibling) mesh = m_pSibling->GetMesh();
	if (m_pChild) mesh = m_pChild->GetMesh();
	return mesh;
}

CMaterial* CGameObject::GetMeterial(int index)
{
	CMaterial* metrial = nullptr;
	if (m_ppMaterials) 	metrial = m_ppMaterials[index];

	if (m_pSibling) metrial = m_pSibling->GetMeterial(index);
	if (m_pChild) metrial = m_pChild->GetMeterial(index);
	return metrial;
}
...
void CGameObject::SetShader(CShader *pShader)
{
	...
	m_ppMaterials[0]->SetMaterialColors(new CMaterialColors());
...
CGameObject* CGameObject::LoadFrameHierarchyFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, FILE* pInFile)
{
	...
	for (; ; )
	{
		...
		else if (!strcmp(pstrToken, "<Transform>:"))
		{
			...
			pGameObject->transform->SetPosition(XMLoadFloat3(&xmf3Position));
			pGameObject->transform->SetScale(xmf3Scale);
		}
		else if (!strcmp(pstrToken, "<TransformMatrix>:"))
		{
			nReads = (UINT)::fread(&pGameObject->transform->m_xmf4x4Transform, sizeof(float), 16, pInFile);
		}
		else if (!strcmp(pstrToken, "<Mesh>:"))
		{
...
{
	return LoadGeometryFromFile(Define::Framework->m_pd3dDevice, Define::Framework->m_pd3dCommandList,Define::SceneManager->GetCurrentScene()->GetGraphicsRootSignature(), pstrFileName);
}
...
void CGameObject::Start()
{
	...
	if (m_pSibling) m_pSibling->Start();
	if (m_pChild) m_pChild->Start();
}
...
void CGameObject::Update()
{
	if (GetActive() == false) return;
	for (const auto& component : ComponetList)
		if(component->GetActive()) component->Update();

	if (m_pSibling) m_pSibling->Update();
...
void CGameObject::LateUpdate()
{
	if (GetActive() == false) return;
	for (const auto& component : ComponetList)
		if (component->GetActive()) component->LateUpdate();

	if (m_pSibling) m_pSibling->LateUpdate();
	if (m_pChild) m_pChild->LateUpdate();
}
...
void CGameObject::Enable()
{
	...
	if (m_pSibling) m_pSibling->Enable();
	if (m_pChild) m_pChild->Enable();
}
...
void CGameObject::Disable()
{
	...
	if (m_pSibling) m_pSibling->Disable();
	if (m_pChild) m_pChild->Disable();
}

void CGameObject::Collision(CCollider& other)
{
	if (GetActive() == false) return;
	if(collider) collider->CollisionCall(*this->collider, other);
	for (const auto& component : ComponetList)
		if (component->GetActive()) component->OnCollision(other);

	if (m_pSibling) m_pSibling->Collision(other);
	if (m_pChild) m_pChild->Collision(other);
}

bool CGameObject::CheckCollision(CCollider& other)
{
	return collider->bound->Intersects(*other.bound);
}

void CGameObject::AddComponet(CComponent* component)
{
	ComponetList.emplace_back(component);
}

CGameObject* CGameObject::GetChilde(string name)
{
	CGameObject* childe = nullptr;
	if (m_pSibling)
	{
		if (m_pSibling->name._Equal(name) || m_pSibling->name._Equal(m_pstrFrameName)) childe = m_pSibling;
		else childe = m_pSibling->GetChilde(name);

		if (childe) return childe;
	}
	if (m_pChild)
	{
		if (m_pChild->name._Equal(name) || m_pChild->name._Equal(m_pstrFrameName)) childe = m_pChild;
		else childe = m_pChild->GetChilde(name);
...
void CGameObject::SetAllColor(XMVECTOR& color)
{
	for (int i = 0; i < m_nMaterials; i++)
		m_ppMaterials[i]->SetColor(color);

	if (m_pSibling) m_pSibling->SetAllColor(color);
	if (m_pChild) m_pChild->SetAllColor(color);
}
